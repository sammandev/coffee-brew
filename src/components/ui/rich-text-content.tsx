...
export function RichTextContent({ className, html }: RichTextContentProps) {
	...
	return (
		<div
			className={cn(
				"space-y-2 text-sm text-(--foreground)/90 [&_a]:text-(--accent) [&_a]:underline [&_blockquote]:border-l-2 [&_blockquote]:border-(--border) [&_blockquote]:pl-3 [&_li]:ml-4 [&_ol]:list-decimal [&_p]:leading-relaxed [&_ul]:list-disc",
				className,
			)}

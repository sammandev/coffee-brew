...
export default async function DashboardPage() {
	const [session, { locale, t }] = await Promise.all([requireRole(), getServerI18n()]);
		...
		<div className="space-y-6">
			<header className="flex flex-wrap items-center justify-between gap-4">
				...
				<div className="flex gap-3">
					<Link href="/dashboard/profile" className="rounded-full border px-4 py-2 text-sm font-semibold">
						{t("dashboard.profile")}
					</Link>
					<Link
